...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.hayaty.kxmpzq"
    minSdk = 24
    targetSdk = 36
    versionCode = 1
    versionName = "1.0"
...
tasks.register("copyApkOutputs") {
  ...
  doLast {
    ...
    if (apkFile.exists()) {
      ...
      println("APK successfully copied to both destinations: ${buildOutputsDir.path} and ${apkDownloadDir.path}")
    } else {
      error("Source APK not found at ${apkFile.absolutePath}")
    }
  }
}
...
tasks.register("printApkSize") {
  doLast {
    ...
    if (apkFile.exists()) {
      println("Apk Size: ${apkFile.length()} bytes (${apkFile.length() / (1024 * 1024).toFloat()} MB)")
    } else {
