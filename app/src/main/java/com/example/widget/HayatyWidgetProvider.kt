...
private suspend fun updateAppWidget(
    ...
    try {
        ...
        val pendingIntent = PendingIntent.getActivity(
            context,
            appWidgetId * 1000,  // Use appWidgetId to make unique
            intent,
        ...
        val pendingCity = PendingIntent.getBroadcast(
            context,
            appWidgetId * 1000 + 100,  // Use appWidgetId to make unique
            cityIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
        ...
        val pendingTasbih = PendingIntent.getBroadcast(
            context,
            appWidgetId * 1000 + 200,  // Use appWidgetId to make unique
            tasbihIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
...
private fun getNextPrayerTime(context: Context): Pair<String, String> {
    return try {
        ...
        // Fixed: Use proper time comparison instead of string comparison
...
private fun compareTime(time1: String, time2: String): Int {
    return try {
        val sdf = SimpleDateFormat("HH:mm", Locale.US)
        val date1 = sdf.parse(time1) ?: return 0
        val date2 = sdf.parse(time2) ?: return 0
